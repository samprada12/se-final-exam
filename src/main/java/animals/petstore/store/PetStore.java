...
public class PetStore {
    ...
    // ------------------------------------------------------------
    // FIXED INVENTORY – IDs MATCH YOUR TESTS
    // ------------------------------------------------------------
    public void init() {
        petsForSale.clear();

        this.addPetInventoryItem(new Dog(
                AnimalType.DOMESTIC, Skin.FUR, Gender.MALE, Breed.MALTESE,
                new BigDecimal("750.00"), 1));

        this.addPetInventoryItem(new Dog(
                AnimalType.DOMESTIC, Skin.FUR, Gender.MALE, Breed.POODLE,
                new BigDecimal("650.00"), 2));

        this.addPetInventoryItem(new Dog(
                AnimalType.DOMESTIC, Skin.HAIR, Gender.MALE, Breed.GERMAN_SHEPARD,
                new BigDecimal("50.00"), 3));

        this.addPetInventoryItem(new Cat(
                AnimalType.DOMESTIC, Skin.HAIR, Gender.MALE, Breed.BURMESE,
                new BigDecimal("65.00"), 1));

        this.addPetInventoryItem(new Cat(
                AnimalType.DOMESTIC, Skin.UNKNOWN, Gender.FEMALE, Breed.SPHYNX,
    ...
    // ------------------------------------------------------------
    // FIXED REMOVE-PET LOGIC (NO DUPLICATION)
    // ------------------------------------------------------------
    ...
    private Dog identifySoldDogFromInventory(Dog soldDog)
        ...
        List<Pet> dogs = this.petsForSale.stream()
                .filter(p -> (p instanceof Dog &&
                        p.getPetStoreId() == soldDog.getPetStoreId()))
                .collect(Collectors.toList());

        if (dogs.size() == 1) return (Dog) dogs.get(0);
    ...
    private Cat identifySoldCatFromInventory(Cat soldCat)
        ...
        List<Pet> cats = this.petsForSale.stream()
                .filter(p -> (p instanceof Cat &&
                        p.getPetStoreId() == soldCat.getPetStoreId()))
                .collect(Collectors.toList());

        if (cats.size() == 1) return (Cat) cats.get(0);
    ...
    private Snake identifySoldSnakeFromInventory(Snake soldSnake)
        ...
        List<Pet> snakes = this.petsForSale.stream()
                .filter(p -> (p instanceof Snake &&
                        p.getPetStoreId() == soldSnake.getPetStoreId()))
                .collect(Collectors.toList());

        if (snakes.size() == 1) return (Snake) snakes.get(0);
